...
export default function ReviewPage() {
  ...
  useEffect(() => {
    ...
    if (selectedPlanType === "weekly") {
      ...
      planId = oneOffPlan?._id;
    }

    if (!planId) {
      setSelectPlanError("Plan not found");
      return;
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    // Validate subscription charge meets Stripe minimum (£0.30 for GBP) — skip for gym users (bulk orders may have small recurring charges)
    if (selectedPlanType !== "one-time" && calculatedCharge < 0.30 && !isGymUser) {
                      ...
                      <div className={styles.orderItemRight}>
                        ...
                        {selectedPlanType === "one-time" ? (
                          ...
                          <button
                            ...
                            onClick={() => {
                              sessionStorage.setItem("sk_returning_from_checkout", "1");
                              router.push("/menu");
                            }}
                            aria-label="Edit"
                            title="Edit weekly meals"
                          >
                            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
